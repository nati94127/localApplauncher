...
function renderApps() {
  ...
  if (filteredApps.length === 0) {
    ...
    filteredApps.forEach((app) => {
      ...
      appCard.innerHTML = `
  <div class="app-menu-container s absolute top-2 right-2 z-10">
  ...
  <h3 class="font-semibold mb-1 text-center truncate truncate  overflow-hidden text-ellipsis block max-w-[8rem] text-white text-base">${
    app.name
  }</h3>
...
function renderPinnedApps() {
  ...
  pinnedApps.forEach((app) => {
    ...
    pinnedApp.innerHTML = `
      <div class="w-6 h-6 mr-3 rounded-md truncate overflow-hidden">
        <img src="${app.iconUrl}" alt="${app.name}" onerror="this.src='icons/icons8-windows-os-48.png';">
      </div>
      <span class="truncate text-xs overflow-hidden text-ellipsis block max-w-[8rem]">
  ${app.name}
</span>
    `;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  categoryTabs.forEach((tab) => {
    tab.addEventListener("click", function () {
      ...
      categoryTabs.forEach((t) => {
        ...
        t.classList.add("bg-[#000]", "text-gray-400");
      });

      // Add active class to clicked tab
      this.classList.add("active", "bg-[#1d1d1d]", "text-white");
      this.classList.remove("bg-[#000]", "text-gray-400");
  ...
  let selectedAppPath = ""; // Global-scoped variable to hold full path
  ...
  document.querySelectorAll(".icon-gallery-tab").forEach((tab) => {
    tab.addEventListener("click", function () {
      ...
      document.querySelectorAll(".icon-gallery-tab").forEach((t) => {
        t.classList.remove("active", "bg-[#0a0a0a]", "text-white");
        t.classList.add("bg-[#000]", "text-gray-400");
      });

      // Add active class to clicked tab
      this.classList.add("active", "bg-[#00371b]", "text-white");
      this.classList.remove("bg-[#000]", "text-gray-400");
